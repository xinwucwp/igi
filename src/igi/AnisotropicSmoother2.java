...
public class AnisotropicSmoother2 {

  public AnisotropicSmoother2(float sigma1, float sigma2, float[][] wp, Tensors2 et) {
    _wp  = wp;
    _et2 = et;
    _sigma1 = sigma1;
    _sigma2 = sigma2;
  ...
  // private
  private float[][] _wp;
  private Tensors2 _et2;

  private float _sigma1,_sigma2;

  // Smoothing for dimension 1.
  private static void smooth(Tensors2 et, float sigma, float[][] s, float[][] x) {
    if (sigma<1.0f)
      return;
    int n2 = x.length;
    int n1 = x[0].length;
    float c = 0.5f*sigma*sigma;
    float[][] st = fillfloat(1.0f,n1,n2);
    float[][] yt = fillfloat(0.0f,n1,n2);
    if(s!=null) {
      st = copy(s);
    }
    LocalSmoothingFilter lsf = new LocalSmoothingFilter();
    lsf.applySmoothS(x,yt);
    lsf.apply(et,c,st,yt,x);
  }
  /*
